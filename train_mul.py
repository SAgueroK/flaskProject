...
MEMORY_WARMUP_SIZE = 20  # replay_memory 里需要预存一些经验数据，再开启训练
...
class Model(parl.Model):
    ...
    def network(self, functions, act_dim):
        # 3层全连接网络
        self.f1 = layers.fc(size=HID1_SIZE, act=functions)
        self.f2 = layers.fc(size=HID2_SIZE, act=functions)
        self.f3 = layers.fc(size=act_dim, act=None)
...
# 训练一个episode
def run_episode(envs, agent, rpm, episode, action_dim=16, process_num=1):
    total_reward = 0
...
def run(functions):
    ...
    # 根据parl框架构建agent
    model = Model()
    model.network(functions, act_dim=action_dim)
